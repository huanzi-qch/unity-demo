...
namespace BestHTTP
{
    ...
    //huanzi修改
    public delegate void OnRequestErrorDelegate(Exception exception, HTTPRequestStates requestStates);//请求错误时的回调函数
    public delegate void OnRequestSuccessDelegate(HTTPRequest originalRequest, HTTPResponse response, Dictionary<string,List<string>> responseHeader);//请求没有错误时的回调函数
    ...
    public sealed class HTTPRequest : IEnumerator, IEnumerator<HTTPRequest>
    {
        ...
        /// huanzi修改，网络请求错误时执行
        /// </summary>
        public OnRequestErrorDelegate ErrorCallback { get; set; }
        
        /// <summary>
        /// huanzi修改，网络请求成功时执行
        /// </summary>
        public OnRequestSuccessDelegate SuccessCallback { get; set; }
        ...
        /// <param name="successCallback">执行成功时回调函数</param>
        /// <param name="errorCallback">执行失败时回调函数</param>
        public HTTPRequest(Uri uri, OnRequestSuccessDelegate successCallback, OnRequestErrorDelegate errorCallback=null, HTTPMethods methodType = HTTPMethods.Get)
        ...
        internal void CallCallback()
        {
            try
            {
                ...
                {
                    if (HTTPRequestStates.Finished == this.State && Response != null && Response.IsSuccess && !String.IsNullOrEmpty(Response.DataAsText))
                        this.SuccessCallback(this, Response, Response.Headers);
                    else if (this.ErrorCallback != null)
                    {
                        ...
                        switch (this.State)
                        {
                            ...
                            case HTTPRequestStates.Finished:
                                ...
                                if (Response != null)
                                {
                                    var errorMsg = string.Format("Request finished Successfully, but the server sent an error. Status Code: {0}-{1} Message: {2}",
                                        Response.StatusCode,
                                        Response.Message,
                                        Response.DataAsText);
                                
                                    reqError = new Exception(errorMsg);
                                }
                                else
                                {
                                    reqError = new Exception("Request finished Successfully, but the server sent an error");
                                }

                                break;

                            case HTTPRequestStates.Error:
                                var errorMsg2 = "Request Finished with Error! " + (this.Exception != null ? (this.Exception.Message + "\n" + this.Exception.StackTrace) : "No Exception");
                                reqError = new Exception(errorMsg2);
                                break;
                            
                            case HTTPRequestStates.Aborted:
                                reqError = new Exception("Request Aborted!");
                                break;
                            
                            case HTTPRequestStates.ConnectionTimedOut:
                                reqError = new Exception("Connection Timed Out!");
                                break;

                            case HTTPRequestStates.TimedOut:
                                reqError = new Exception("Processing the request Timed Out!");
                                break;
                            default:
                                reqError = new Exception($"请求网络错误，返回状态为{this.State}");
                                break;
                        }

                        if (this.ErrorCallback == null)
                        {
                            this.ErrorCallback = (Exception exception, HTTPRequestStates requestStates) =>
                            {
                                Debug.Log("网络请求失败：" + exception.Message);
                            };
                        }
                        this.ErrorCallback(reqError, this.State);
                    }
